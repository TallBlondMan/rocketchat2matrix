...
export async function inviteMember(
  ...
  try {
    ...
    if (
      ...
      error.response &&
      error.response.data.errcode === 'M_FORBIDDEN' &&
      error.response.data.error ==='You are not invited to this room.'
    ) {
      log.warn(
        `Cannot accept the invite`
      )
      return
    } else {
      throw error
...
export async function executeAndHandleMissingMember(
  ...
  try {
    ...
    if (
      ...
      // Skip if no invite to room???
      try {
        await axios.post(
          `/_matrix/client/v3/join/${matrixRoomId}`,
          {}, 
          userSessionOptions || '')
      } catch (error){
        if (
          ...
          log.warn(`User not invited to room ${matrixRoomId}, skipping`)
          return
        }
      }

      // Skip for outside room??
      try {
        await axios.post(
          `/_matrix/client/v3/join/${matrixRoomId}`,
          {}, 
        ...
        if (
          error instanceof AxiosError &&
          error.response &&
          error.response.data.errcode === 'M_UNKNOWN' &&
          error.response.data.error === "Can't join remote room because no servers that are in the room have been provided."
        ) {
          log.warn(`No invitation although it's public room ${matrixRoomId}, skipping`)
          return
        }
      }
