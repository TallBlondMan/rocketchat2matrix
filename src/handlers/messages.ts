...
export async function handle(rcMessage: RcMessage): Promise<void> {
  ...
  // Skip message if msg is empty, Would be best to take into account ""attachments":[{"text""
  // as message might be empty but some content is still there
  if (!rcMessage.msg) {
    log.warn(
      'The message is empty - probably only attachements, skipping'
    )
    return
  }
